...
STATIC mp_obj_t connection_filter_by_id(mp_obj_t enabled) {
    tranceiver_enable_filter_by_id(mp_obj_get_int(enabled));
    return mp_const_none;
}
MP_DEFINE_CONST_FUN_OBJ_1(connection_filter_by_id_obj, connection_filter_by_id);


STATIC mp_obj_t connection_set_channel(mp_obj_t channel) {
    tranceiver_set_channel(mp_obj_get_int(channel));
    return mp_const_none;
}
MP_DEFINE_CONST_FUN_OBJ_1(connection_set_channel_obj, connection_set_channel);

STATIC mp_obj_t connection_set_power(mp_obj_t power) {
    tranceiver_set_power(mp_obj_get_int(power));
    return mp_const_none;
}
MP_DEFINE_CONST_FUN_OBJ_1(connection_set_power_obj, connection_set_power);


STATIC mp_obj_t connection_set_id(mp_obj_t id_bytes) {
    mp_obj_t* id_py;
...
MP_DEFINE_CONST_FUN_OBJ_1(connection_set_id_obj, connection_set_id);

STATIC mp_obj_t connection_send_control_packet(mp_obj_t channels) {
...
MP_DEFINE_CONST_FUN_OBJ_1(connection_send_control_packet_obj, connection_send_control_packet);

STATIC mp_obj_t connection_send_8266_control_packet(mp_obj_t channels) {
    mp_obj_t* channel_values_py;
    size_t num_channels = 0;
    mp_obj_get_array(channels, &num_channels, &channel_values_py);
    if (num_channels > 12){
...
MP_DEFINE_CONST_FUN_OBJ_1(connection_send_8266_control_packet_obj, connection_send_8266_control_packet);
...
STATIC const mp_map_elem_t connection_globals_table[] = {
    ...
    { MP_OBJ_NEW_QSTR(MP_QSTR_filter_by_id), (mp_obj_t)&connection_filter_by_id_obj },
    { MP_OBJ_NEW_QSTR(MP_QSTR_get_latest_packet), (mp_obj_t)&connection_get_latest_packet_obj },
    { MP_OBJ_NEW_QSTR(MP_QSTR_set_channel), (mp_obj_t)&connection_set_channel_obj },
    { MP_OBJ_NEW_QSTR(MP_QSTR_set_power), (mp_obj_t)&connection_set_power },
    { MP_OBJ_NEW_QSTR(MP_QSTR_set_id), (mp_obj_t)&connection_set_id_obj },
    { MP_OBJ_NEW_QSTR(MP_QSTR_send_control_packet), (mp_obj_t)&connection_send_control_packet_obj },
    { MP_OBJ_NEW_QSTR(MP_QSTR_send_8266_control_packet), (mp_obj_t)&connection_send_8266_control_packet_obj },
